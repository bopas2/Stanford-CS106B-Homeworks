...
void random(Grid<string> &board, int row, int col);
...
int main() {
    ...
    cout << "XXWelcome to the CS 106B Game of Life," << endl;
    ...
    int randfile = 0;
    while(randfile = 0) {
        random = getLine("Random? (y/n) ");
        random = toLowerCase(random);
        if(random.compare("y") == 0) {
                int r = getInteger("Number of rows?","Invalid input, try again ");
                int c = getInteger("Number of rows?","Invalid input, try again ");
                random(lifeBoard,r,c);
                randfile = 1; fileDNE = 1;
        }
        if(random.compare("n") == 0) {
            randfile = 1;
        }
